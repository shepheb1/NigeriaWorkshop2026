...
Note that under the model with random intercepts only, the estimated correlation is $\frac{`r as.data.frame(VarCorr(lme_m1))$vcov[1]`}{`r as.data.frame(VarCorr(lme_m1))$vcov[1]` + `r as.data.frame(VarCorr(lme_m1))$vcov[2]`} = `r round(as.data.frame(VarCorr(lme_m1))$vcov[1]/(as.data.frame(VarCorr(lme_m1))$vcov[1]+as.data.frame(VarCorr(lme_m1))$vcov[2]), 2)`$ (see slide 11 of the notes for day 3).
...
I also calculated the estimated correlation matrix induced by the model with random intercepts and slopes (see slide 13 of the notes for day 3).

```{r corr_plot, echo=FALSE}
D_11 <- as.data.frame(VarCorr(lme_m4))[1,4]
D_22 <- as.data.frame(VarCorr(lme_m4))[2,4]
D_12 <- as.data.frame(VarCorr(lme_m4))[3,4]
sigmasq <- as.data.frame(VarCorr(lme_m4))[4,4]

tt <- 0:10
tp <- 0:10

xx <- data.frame(expand.grid(tt=tt, tp=tp))

xx <- within(xx, {
  Correlation <- (D_11 + D_12*(tt + tp) + D_22*tt*tp)/(sqrt(D_11 + 2*D_12*tt + D_22*tt^2 + sigmasq)*sqrt(sqrt(D_11 + 2*D_12*tp + D_22*tp^2 + sigmasq)))
  Correlation <- ifelse(tt==tp, NA, Correlation)})

ggplot(xx, aes(tt, tp, fill=Correlation)) +
  geom_tile() +
  scale_x_continuous(name="Year", breaks=0:10) +
  scale_y_reverse(name="Year", breaks=0:10) + 
  theme(panel.grid.minor=element_blank())
```
